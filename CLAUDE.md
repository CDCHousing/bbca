...
- Campaign mail uses `renderPlainEmailShell()` (letter-style: no navy masthead, just text on white
  plus a small contact footer) and is sent **multipart** — `html` + a `text` part generated by
  `htmlToPlainText()`. Both exist to keep Gmail from filing member mail under Promotions; a
  full-width branded header reads as a newsletter to the tab classifier. Booking confirmations keep
  the branded `renderEmailShell()`. Contact details for both footers live in the `CONTACT` const in
  `src/lib/email.ts` — keep it in sync with `src/components/Footer.tsx`.
- Gmail tab placement cannot be forced from the server; there is no header for it. Landing in
  Updates is normal and fine for member correspondence. The only reliable fix is per-recipient —
  they drag one message to Primary, or add the sender to Contacts.
