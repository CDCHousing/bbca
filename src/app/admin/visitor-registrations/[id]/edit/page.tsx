...
export default function EditVisitorRegistrationPage({
  params,
}: {
  ...
  useEffect(() => {
    async function loadRegistration() {
      try {
        const res = await fetch(`/api/admin/visitor-registrations/${id}`);
        if (!res.ok) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push("/admin/visitor-registrations");
      router.refresh();
    } catch {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold text-gray-900">Edit Registration</h1>
      </div>
